<?php
// ══════════════════════════════════════════════════════════════
// index.php — HostingAura Speed Test
// ══════════════════════════════════════════════════════════════
session_start();
...
.form-group input,
.form-group select,
.form-group textarea{width:100%;background:#0c0c1c;border:1px solid #1e1e3a;border-radius:8px;padding:10px 12px;font-size:.85rem;color:#e2e8f0;outline:none;transition:.2s;font-family:inherit}
...
<!-- ══ REPORT ISSUE LINK (shown only after a test) ═══════════ -->
...
<div class="modal-overlay" id="modal-report">
  <div class="modal">
    ...
    <!-- Read-only snapshot of the current result -->
    <div class="rpt-snap" id="rpt-snap">
      <div class="rpt-snap-col">
        <span class="rpt-snap-lbl">Test ID</span>
        <span class="rpt-snap-val" id="rpt-snap-id">–</span>
      </div>
      <div class="rpt-snap-col">
        <span class="rpt-snap-lbl">Download</span>
        <span class="rpt-snap-val" id="rpt-snap-dl">–</span>
      </div>
      <div class="rpt-snap-col">
        <span class="rpt-snap-lbl">Upload</span>
        <span class="rpt-snap-val" id="rpt-snap-ul">–</span>
      </div>
      <div class="rpt-snap-col">
        <span class="rpt-snap-lbl">Ping</span>
        <span class="rpt-snap-val" id="rpt-snap-ping">–</span>
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <textarea id="rpt-desc" maxlength="1000"
        placeholder="Describe the issue — e.g. my actual speed is ~500 Mbps but the test showed 2 Mbps…"
        oninput="updateCharHint()"></textarea>
...
<!-- ════════════════════════════════════════════════════════════
     MODAL — REGISTER STEP 2 (OTP + PASSWORD)
═════════════════════════════════════════════════════════════ -->
...
// PHP-injected: logged-in user contact (or null for guests)
const currentUserContact = <?= $isLoggedIn
    ? json_encode(htmlspecialchars($userContact, ENT_QUOTES))
    : 'null' ?>;
...
function detectDevice() {
  ...
  if (/iPad/.test(ua)) {
    return `${browser} on ${detectiPadModel()}`;
  } else if (/iPhone/.test(ua)) {
    return `${browser} on ${detectiPhoneModel()}`;
    ...
    return `${browser} on ${m ? m[1] : 'Android'}`;
  } else if (/Windows NT/.test(ua)) {
    const v   = parseFloat((ua.match(/Windows NT (\d+\.\d+)/) || [0, 0])[1]);
    const win = v >= 10 ? 'Windows 11' : v >= 6.3 ? 'Windows 10' : v >= 6.2 ? 'Windows 8' : 'Windows';
    return `${browser} on ${win}`;
  } else if (/CrOS/.test(ua)) {
    return `${browser} on Chromebook`;
...
function detectiPadModel() {
  ...
  if (w === 810  && h === 1080 && r === 2) return 'iPad';
  return 'iPad';
}
...
function renderTurnstileWidget(containerId, callbackName) {
  ...
  if (typeof turnstile === 'undefined') { console.error('Turnstile not loaded yet'); return null; }
  try {
    ...
    console.log(`✅ Rendered ${containerId}:`, id);
    return id;
  } catch(e) {
...
async function sendOtp() {
  ...
  try {
    const r = await fetch('sent_otp.php', {
      ...
      body: JSON.stringify({ contact, type: otpType, turnstile_token: registerTurnstileToken })
    });
    ...
    if (d.status === 'success') {
      ...
      if (typeof turnstile !== 'undefined' && registerWidgetId !== null) {
        turnstile.reset(registerWidgetId); registerTurnstileToken = null; btn.disabled = true;
      }
    }
  } catch(e) {
...
async function submitLogin() {
  ...
  try {
    const r = await fetch('login.php', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ contact, password: pass, turnstile_token: loginTurnstileToken })
    });
    const d = await r.json();
    ...
    if (d.status === 'success') {
      ...
      if (typeof turnstile !== 'undefined' && loginWidgetId !== null) {
        turnstile.reset(loginWidgetId); loginTurnstileToken = null; btn.disabled = true;
      }
    }
  } catch(e) {
...
function startLockoutTimer(seconds, message) {
  ...
  function updateDisplay(secs) {
    ...
    const s    = secs % 60;
    msgEl.innerHTML = `<div class="lockout-msg">${message}</div><div class="lockout-timer">${mins}:${String(s).padStart(2,'0')}</div>`;
...
async function submitForgotPassword() {
  ...
  try {
    const r = await fetch('forgot_password.php', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ contact, type: forgotType, turnstile_token: forgotTurnstileToken })
    });
    const d = await r.json();
    if (d.status === 'success') {
      ...
      if (typeof turnstile !== 'undefined' && forgotWidgetId !== null) {
        turnstile.reset(forgotWidgetId); forgotTurnstileToken = null; btn.disabled = true;
      }
    }
  } catch(e) {
...
function openReportModal() {
  ...
  // Auto-fill contact for logged-in users; lock the field so they know it's set
  ...
  contactEl.title      = currentUserContact ? 'Auto-filled from your account' : '';
...
async function submitReport() {
  ...
  if (!desc) {
    setMsg('rpt-msg', 'Please describe the issue before submitting.', 'error');
    return;
  }
  if (desc.length > 1000) {
...
async function fetchInfo() {
  try {
    ...
    try {
      ...
      if (ipD && ipD.org) { document.getElementById('v-isp').textContent = ipD.org; ispFound = true; }
    } catch(e) {}
    ...
    if (!ispFound) {
      try {
        ...
        if (ipD2 && ipD2.isp) { document.getElementById('v-isp').textContent = ipD2.isp; ispFound = true; }
      } catch(e) {}
    }
      ...
      try {
        ...
        while (true) {
          ...
          if (now - lastUpdate > 200) {
            ...
            if (elapsed > 0.5) cb((downloaded * 8) / (elapsed * 1000000));
            lastUpdate = now;
          }
        }
      } catch(e) { break; }
...
async function runTest() {
  ...
  try {
    ...
    // Store readable values for the report modal
    lastResult.dl   = dlSpeed > 100 ? Math.round(dlSpeed).toString() : dlSpeed.toFixed(1);
    lastResult.ul   = ulSpeed > 100 ? Math.round(ulSpeed).toString() : ulSpeed.toFixed(1);
    ...
    // Show the report link now that we have a result
