...
#share-btn{background:#1e1e3a;color:#94a3b8;border:1px solid #334155;border-radius:999px;padding:12px 24px;font-size:.9rem;cursor:pointer;display:none}
...
<div class="wrap" id="gauge-wrap">
  <svg viewBox="0 0 300 300">
    <defs>
      <linearGradient id="gDL" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#6366f1"/><stop offset="100%" stop-color="#06b6d4"/></linearGradient>
      <linearGradient id="gUL" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#7c3aed"/><stop offset="100%" stop-color="#ec4899"/></linearGradient>
    </defs>
    <circle cx="150" cy="150" r="110" fill="none" stroke="#131325" stroke-width="16" stroke-dasharray="518.36 691.15" transform="rotate(135 150 150)"/>
    <circle id="arc" cx="150" cy="150" r="110" fill="none" stroke="url(#gDL)" stroke-width="16" stroke-linecap="round" stroke-dasharray="518.36 691.15" stroke-dashoffset="518.36" transform="rotate(135 150 150)" style="transition:stroke-dashoffset .15s ease-out"/>
  </svg>
  <div class="ci">
    <div id="val">0</div>
...
<div class="cards" id="cards-wrap">
  <div class="card" id="cd"><div class="cv" id="rd">–</div><div class="cn">Down</div></div>
  <div class="card" id="cu"><div class="cv" id="ru">–</div><div class="cn">Up</div></div>
  <div class="card" id="cp"><div class="cv" id="rp">–</div><div class="cn">Ping</div></div>
</div>

<div class="mbbar">📦 Data Used: <span class="mbv" id="mbv">0.00</span> MB</div>

<div class="privacy-notice">
  By running this test, you agree that we collect your IP address, ISP, location, and test results for service improvement. <a href="privacy.html">Privacy Policy</a>
</div>
...
<div class="modal-overlay" id="modal-register">
  <div class="modal">
    ...
    <button class="form-submit" id="reg-send-btn" onclick="sendOtp()">Send Verification Code</button>
...
function clearMessages() {
  ['login-msg','reg-msg','verify-msg'].forEach(id => {
...
function hideTestId() { document.getElementById('test-id-box').style.display = 'none'; }
...
async function measurePing() {
    ...
    for(let i=0; i<6; i++) {
        ...
        try { await fetch(DL_HOST+'?size=0.01&t='+t0,{cache:'no-store'}); if(i>0) pings.push(performance.now()-t0); }
        catch(e) { if(i>0) pings.push(performance.now()-t0); }
    }
    return pings.length > 0 ? Math.min(...pings) : 0;
}
async function measureDownload(cb) {
    const duration=8000, start=performance.now();
    let downloaded=0, stop=false, lastUpdate=performance.now();
        ...
        while(!stop) {
            try {
                ...
                while(true) {
                    ...
                    if(done||stop) break;
                    downloaded+=value.length; totalBytesUsed+=value.length;
...
async function measureUpload(cb) {
    ...
    let uploaded=0, stop=false, lastUpdate=performance.now(), speedSamples=[];
    setTimeout(()=>stop=true, duration);
    async function worker() {
        while(!stop) {
            try {
                await new Promise((res) => {
                    ...
                    xhr.upload.onprogress=e=>{
                        const chunk=e.loaded-last; last=e.loaded;
                        ...
                        if(now-lastUpdate>200) {
                            ...
                            if(elapsed>0.5) {
                                ...
                                speedSamples.push(s); if(speedSamples.length>5) speedSamples.shift();
                                cb(speedSamples.reduce((a,b)=>a+b,0)/speedSamples.length);
                            }
                            lastUpdate=now;
                        }
                    };
                    xhr.onload=()=>res(); xhr.onerror=()=>res(); xhr.ontimeout=()=>res();
                    xhr.send(payload);
                });
            } catch(e) { break; }
        }
...
async function runTest() {
    ...
    sBtn.style.display="none"; totalBytesUsed=0;
    showGaugeResetResults(); hideTestId();
    ...
    try {
        ...
        } catch(saveError) { console.error('Save error:',saveError); }
        ...
        b.disabled=false; b.textContent="Start Speed Test";
    }
}
